...
# 构建二分类器
dtc = tree.DecisionTreeClassifier(max_depth=15,min_samples_leaf=2,min_samples_split=5,random_state=10)
weight = []
for each in y_train:
   weight.append(0.07) if each == 0 else weight.append(0.1)

dtc.fit(X_train,y_train,sample_weight=weight)
...
for row in reader:
    ...
    tmp.append(row[5])
    tmp.append(row[6])
    tmp.append(dtc.predict([row[7:-1]])[0])  # predict
    tmp.append(gbm0.predict([row[7:-1]])[0])  # predict
    tmp.append(rf0.predict([row[7:-1]])[0])  # predict
    tmp.append(int(row[-1]))  # predict
    flag=0
    if int(row[-1])==int(dtc.predict([row[7:-1]])[0]):
        flag=flag+1
    if int(row[-1])==int(gbm0.predict([row[7:-1]])[0]):
        flag=flag+1
    if int(row[-1])==int(rf0.predict([row[7:-1]])[0]):
        flag=flag+1
    if flag>=2:
        true=true+1
    else:
